...
{
    use HasFactory;
    private static $products=[], $product, $category, $combinedAllCategoryOfSameId=[];
    ...
    public static function getProductById($id){
        ...
        foreach (self::$product as $item) {
            if ($item['id']==$id){
                self::$category= Category::getCategoryById($item['category_id']);
    ...
    public static function getProductByCategoryId($id){
        ...
        foreach (self::$products as self::$product) {
            if(self::$product['category_id']==$id){
                array_push(self::$combinedAllCategoryOfSameId, self::$product);
            }
        }
        return self::$combinedAllCategoryOfSameId;
    }
}
